...
def fit(pre_features_list, act_Y, test_features, start_time):
   ...
   xs = tf.placeholder(tf.float32, [None, 25])  # 原始数据的维度：16
   ...
   # 初始化算法模型
   ## conv1 layer ##第一卷积层
   W_conv1 = weight_variable([2, 2, 1, 25])  # patch 2x2, in size 1, out size 32,每个像素变成32个像素，就是变厚的过程
   ...
   ## conv2 layer ##第二卷积层
   W_conv2 = weight_variable([2, 2, 25, 50])  # patch 2x2, in size 32, out size 64
   b_conv2 = bias_variable([50])
   h_conv2 = tf.nn.relu(conv2d(h_conv1, W_conv2) + b_conv2)  # 输入第一层的处理结果 输出shape 4*4*64

   ## fc1 layer ##  full connection 全连接层
   W_fc1 = weight_variable([5 * 5 * 50, 625])  # 4x4 ，高度为64的三维图片，然后把它拉成512长的一维数组
   b_fc1 = bias_variable([625])

   h_pool2_flat = tf.reshape(h_conv2, [-1, 5 * 5 * 50])  # 把4*4，高度为64的三维图片拉成一维数组 降维处理
   h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)

   h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)  # 把数组中扔掉比例为keep_prob的元素
   ## fc2 layer ## full connection
   W_fc2 = weight_variable([625, 1])  # 512长的一维数组压缩为长度为1的数组
...
# 33个特征
feature = [x for x in train.columns if x not in ['TERMINALNO','Y','hour_count_max','night_count_max','user_distance',
                                                  'hour_18_per', 'hour_17_per', 'night__day_delta', 'hour_15_per',
                                                 'hour_5_per', 'hour_14_per', 'hour_8_per', 'hour_9_per',
                                                 'user_lon_mean','user_lon_std','user_lat_mean','user_lat_std','']]
...
result.loc[:, 'Pred'] = result['Pred']#.map(lambda x: abs(x/50) if x < 0 else x )
# df = pd.DataFrame(pre_result, columns=['Id', 'Pred'])
